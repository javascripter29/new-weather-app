...
export const ForecastHourList = ({
  ...
  const hourlyToday = useMemo(() => {
    ...
    if (!list.length || nowUnixSeconds == null) return [];
      ...
      <div className={styles.row} role="list">
        {list.length ? (
          list.map((item) => {
            ...
            const tempValue =
              tempC != null ? Math.round(tempFromC(tempC, units)) : "Нет данных";
            ...
            return (
              <div key={item.dt} className={styles.item} role="listitem">
                ...
                <div className={styles.temp}>
                  {tempValue} {tempValue === "Нет данных" ? "" : units === "f" ? "°F" : "°C"}
                </div>
