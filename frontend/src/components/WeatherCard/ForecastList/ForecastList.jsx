...
export const ForecastList = ({ timezoneOffsetSeconds, forecast, units }) => {
  const days = useMemo(() => {
    ...
    const results = Array.from(groups.values()).map((group) => {
      ...
      return {
        dayKey: group.key,
        date: dtFirst ? new Date((dtFirst + timezoneOffsetSeconds) * 1000) : null,
        maxTempC: temps.length ? Math.max(...temps) : null,
        minTempC: mins.length ? Math.min(...mins) : null,
        popAvg: pops.length ? pops.reduce((a, b) => a + b, 0) / pops.length : null,
        iconCode: items[0]?.weather?.[0]?.icon,
      };
    });
    ...
    <div className={styles.wrap} aria-label="Прогноз на 7 дней">
      {days.length ? (
        days.map((d) => {
          ...
          const maxValue =
            d.maxTempC != null
              ? Math.round(tempFromC(d.maxTempC, units))
              : "Нет данных";
          const minValue =
            d.minTempC != null
              ? Math.round(tempFromC(d.minTempC, units))
              : "Нет данных";
                ...
                <div className={styles.left}>
                  ...
                  <div className={styles.temps}>
                    <span className={styles.max}>
                      {maxValue} {maxValue === "Нет данных" ? "" : tempUnit}
                    </span>
                    <span className={styles.min}>
                      {minValue} {minValue === "Нет данных" ? "" : tempUnit}
                    </span>
                  </div>
                </div>
